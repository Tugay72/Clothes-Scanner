  ...
  const processImage = async (imageUri) => {
    if (imageUri) {
      try {
        ...
        const response = await axios.post("https://clothes-scanner-backend.onrender.com/process-image", formData, {
          headers: { "Content-Type": "multipart/form-data" },
        });
  
        console.log("Response from API:", response.data);
  
        // Safely destructure with fallback defaults
        const {
        ...
        if (dominant_color_name && dominant_color_name.includes(",")) {
          const colorsArray = dominant_color_name.split(/\s*,\s*/).map((color) => {
            return tr_dict[color?.toString()] || color;
          });
  
          const newColorsArray = colorsArray.join(" ve ");
          ...
          const translated_dominant_color_name = tr_dict[dominant_color_name?.toString()] || dominant_color_name;
          turkishString = `
        ...
        Speech.speak(turkishString, { language: "tr-TR" });
